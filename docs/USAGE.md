...
The overlay keeps every non-badge area transparent. Normal text is rendered through a crisp antialiased glyph mask: each visible text pixel samples the desktop directly beneath it and becomes pure black or white independently using linear-RGB relative luminance. Windows background capture uses the physical window rectangle so contrast remains correct on secondary high-DPI displays. One character can therefore be partly black and partly white across a light/dark boundary, including on saturated multicolored backgrounds. Positive `+token` deltas and the rolling value/model animation remain green. Click the `自动` indicator only when a temporary manual black override is needed; click it again to resume automatic mode.
