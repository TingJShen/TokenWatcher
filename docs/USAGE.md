...
The overlay keeps every non-badge area transparent. It samples the desktop under each text field independently and switches only that field between black and white, so neighboring values can use different colors over photos, gradients, and multicolored windows. Positive `+token` deltas and the rolling value/model animation remain green. Click the `自动` indicator only when a temporary manual black/white override is needed; click it again to resume automatic mode.
